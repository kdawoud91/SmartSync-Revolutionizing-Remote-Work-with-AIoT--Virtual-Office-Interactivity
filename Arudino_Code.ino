...
const int lamppin = 8; // the pin that the LED is attached to
const int fanpin=7;
...
void setup() {
  ...
  pixels.fill(pixels.Color(0, color, 0));
  pixels.show();   
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (incomingByte == 'H') {
      digitalWrite(lamppin, LOW); // will turn on the lamp
      pixels.clear();
      pixels.fill(pixels.Color(color, 0, 0));
      pixels.show();    
    }
    if (incomingByte == 'L') {
      digitalWrite(fanpin, HIGH); // will stop the fan
    }
    if (incomingByte == 'A') {
      digitalWrite(fanpin, LOW);
    }
